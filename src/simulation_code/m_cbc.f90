...
module m_cbc
    ...
    abstract interface ! =======================================================
        ...
        subroutine s_compute_abstract_L(dflt_int, lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, dvel_ds, dadv_ds, dpres_dy, dpres_dz, dpres_dx) ! --------------

        integer, intent(IN) :: dflt_int
        real(kind(0d0)), dimension(:), intent(IN) :: lambda, mf, dalpha_rho_ds, dvel_ds, dadv_ds
        real(kind(0d0)), intent(IN) :: rho, c, dpres_ds, dpres_dy, dpres_dz, dpres_dx
    ...
    real(kind(0d0))                            ::      dpres_dx !< Spatial derivatives in x-dir of pressure
    real(kind(0d0))                            ::      dpres_dy !< Spatial derivatives in y-dir of pressure
    real(kind(0d0))                            ::      dpres_dz !< Spatial derivatives in z-dir of pressure
...
contains
    ...
    subroutine s_cbc(q_prim_vf, flux_vf, flux_src_vf, & ! -----------------
       ...
       ! dpres_dx = q_prim_rsx_vf(j, k, r, E_idx)* &
       !           fd_coef_x(j, cbc_loc)

       ! dpres_dy = q_prim_rsy_vf(j, k, r, E_idx)* &
       !            fd_coef_y(j, cbc_loc)
                        
       ! dpres_dz = q_prim_rsz_vf(j, k, r, E_idx)* &
       !           fd_coef_z(j, cbc_loc)    
    ...
    !$acc loop seq
                   ...
                   ! call s_compute_L(dflt_int, lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, dvel_ds, dadv_ds, dpres_dy, dpres_dz, dpres_dx) ! --------------
                    ...
                    else if((cbc_loc == -1 .and. bcxb == -20) .or. (cbc_loc == 1 .and. bcxe == -20)) then  
                        call s_compute_noslip_wall_L(dflt_int, lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, dvel_ds, dadv_ds, dpres_dy, dpres_dz, dpres_dx) ! --------------
                    else  
    ...
    !$acc loop seq
                 ...
                 !   call s_compute_L(dflt_int, lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, dvel_ds, dadv_ds, dpres_dy, dpres_dz, dpres_dx) ! --------------
                    ...
                    else if((cbc_loc == -1 .and. bcyb == -20) .or. (cbc_loc == 1 .and. bcye == -20)) then  
                        call s_compute_noslip_wall_L(dflt_int, lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, dvel_ds, dadv_ds, dpres_dy, dpres_dz, dpres_dx) ! --------------
                    else  
                        call s_compute_supersonic_outflow_L(dflt_int, lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, dvel_ds, dadv_ds) ! --------------
                    end if
    ...
    !$acc loop seq
                   ...
                   ! call s_compute_L(dflt_int, lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, dvel_ds, dadv_ds, dpres_dy, dpres_dz, dpres_dx) ! --------------
                    ...
                    else if((cbc_loc == -1 .and. bczb == -20) .or. (cbc_loc == 1 .and. bcze == -20)) then  
                        call s_compute_noslip_wall_L(dflt_int, lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, dvel_ds, dadv_ds, dpres_dy, dpres_dz, dpres_dx) ! --------------
    ...
    !$acc loop seq
        ...
        ! The reshaping of outputted data and dissociation of the FD and PI
...
!$acc routine seq
        ...
        L(advxe) = lambda(3)*(dpres_ds + rho*c*dvel_ds(dir_idx(1)))
                ...
                !>  The L variables for the slip wall CBC, see pg. 451 of
        !!      Thompson (1990). At the no-slip wall (frictionless wall),
        !!      the normal component of velocity is zero at all times,
        !!      and the transverse velocities is also zero.        
        !!  @param dflt_int Default null integer
        SUBROUTINE s_compute_noslip_wall_L(dflt_int, lambda, L, rho, c, mf, dalpha_rho_ds, dpres_ds, dvel_ds, dadv_ds, dpres_dy, dpres_dz, dpres_dx) ! --------------
!$acc routine seq
        integer, intent(IN) :: dflt_int
        real(kind(0d0)), dimension(3), intent(IN) :: lambda
        real(kind(0d0)), dimension(num_fluids), intent(IN) ::  mf, dalpha_rho_ds, dadv_ds
        ...
        integer :: i
            ...
            L(3) = 0d0 - (dpres_ds/rho)   

            L(4) = 0d0 - (dpres_ds/rho)          

           ! do i = 3, advxe
           !    L(i) = 0d0 - (dpres_ds/rho)
           ! end do
